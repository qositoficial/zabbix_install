...
asteriskCmd() {
  ...
  if [ "$asteriskCacheEnabled" = "true" ]; then
	...
	$sudo docker exec -u root confast-asterisk asterisk -rx "$asteriskCmd" > "$asteriskCacheFile"
      fi
      cat "$asteriskCacheFile"
    else
      #####$sudo asterisk -rx "$asteriskCmd"
      $sudo docker exec -u root confast-asterisk asterisk -rx "$asteriskCmd"
  fi
}
...
calls.longest.channel() {
  ...
  [ -z "$channel" ] && echo 0 || if [ "$channel" != "Message/ast_msg_queue" ]; then
    echo "$channel"
  else
    echo 0
  fi
...
pjsip.registry() {
  ...
  #asteriskCmd "pjsip show registrations" | grep $1 | awk '{print $3}'
}
